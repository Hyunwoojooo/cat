...
class InfoCat extends StatefulWidget {
  final Cat cat;
  const InfoCat({required this.cat, super.key});

  @override
  State<InfoCat> createState() => _InfoCatState();
}
...
class _InfoCatState extends State<InfoCat> {
  ...
  String? originalFurValue;
  String? originalNeuteringValue;
  String? originalEyesColor;
  String? originalAge;
  ...
  void _loadCatData() {
    setState(() {
      // 현재 값 설정
      savedImagePath = widget.cat.image;
      imageDateTime = widget.cat.keyDiscoveryTime;
      locationName = widget.cat.location;
      specialNotes = widget.cat.specialNotes;
      selectedFurValue = widget.cat.furColor;
      selectedAge = widget.cat.age;
      ...
      // 기존 값 저장
      originalImagePath = widget.cat.image;
      originalImageDateTime = widget.cat.keyDiscoveryTime;
      originalLocationName = widget.cat.location;
      originalSpecialNotes = widget.cat.specialNotes;
      originalFurValue = widget.cat.furColor;
      originalAge = widget.cat.age;
      originalEyesColor = widget.cat.eyeColor ?? "선택";
      originalPattern = widget.cat.pattern;
      originalNeuteringValue = widget.cat.isNeutered ? "O" : "X";
      originalDetailLocation = widget.cat.detailLocation;
    });
  }
  ...
  bool get isFormValid {
    ...
    bool hasChanges = savedImagePath != originalImagePath ||
        ...
        selectedFurValue != originalFurValue ||
        selectedAge != originalAge ||
        selectedEyesColor != originalEyesColor ||
        selectedPattern != originalPattern ||
        selectedNeuteringValue != originalNeuteringValue ||
        selectedDetailLocation != originalDetailLocation;
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  controller:
                      TextEditingController(text: selectedFurValue ?? ''),
                ...
                TextField(
                  ...
                  controller:
                      TextEditingController(text: selectedEyesColor ?? ''),
                ...
                _buildDropdown(
                  items: patternItems,
                  value: selectedPattern ?? patternItems[0],
                ...
                _buildDropdown(
                  ...
                  value: selectedNeuteringValue ?? neuteringItems[0],
  ...
  void _showDeleteDialog() {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('고양이 정보 삭제'),
          content: Text('정말로 이 고양이 정보를 삭제하시겠습니까?'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: Text('취소'),
  ...
  Future<void> _deleteCat() async {
    try {
      await CatDatabase.instance.deleteCat(widget.cat.catId);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('고양이 정보가 삭제되었습니다.'),
            duration: Duration(seconds: 2),
          ),
        );
        Navigator.of(context).pop();
